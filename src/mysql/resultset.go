...
// 简化版本的数据
func FormatBinary(f []*Field, values[]interface{}) ([]byte, error) {
	...
	for i := range f {
		...
		case MYSQL_TYPE_TINY:
			if v, ok := values[i].(uint64); ok {
				...
				log.Errorf("Invalid format of short")
			}
			continue

		case MYSQL_TYPE_SHORT, MYSQL_TYPE_YEAR:
			if v, ok := values[i].(uint64); ok {
				binary.LittleEndian.PutUint16(tmp[0:2], uint16(v))
				p = append(p, tmp[0:2]...)
			} else {
				log.Errorf("Invalid format of short")
			}
		...
		case MYSQL_TYPE_INT24, MYSQL_TYPE_LONG:
			if v, ok := values[i].(uint64); ok {
				...
				log.Errorf("Invalid format of int24")
			}
			continue
		...
		case MYSQL_TYPE_LONGLONG:
			if v, ok := values[i].(uint64); ok {
				...
				log.Errorf("Invalid format of int64")
			}
			continue
		case MYSQL_TYPE_DECIMAL, MYSQL_TYPE_NEWDECIMAL, MYSQL_TYPE_VARCHAR,
			MYSQL_TYPE_BIT, MYSQL_TYPE_ENUM, MYSQL_TYPE_SET, MYSQL_TYPE_TINY_BLOB,
			...
			if v, ok := values[i].([]byte); ok {
				data := PutLengthEncodedString(v)
				p = append(p, data...)
			} else if v1, ok1 := values[i].(string); ok1 {
				data := PutLengthEncodedString([]byte(v1))
				p = append(p, data...)
			} else {
				log.Errorf("Invalid format of int64")
			}
